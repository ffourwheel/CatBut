...
| **ATTACK** | `cat_hole_attack` texture. Fast lunge forward with claws extended. พื้นหลังมืดลงประมาณ 35% และคงไว้เฉพาะ cute generated paw/claw cutscene จากนั้นหัวใจที่เสียจะ react ใน HUD | `sfx_cat_attack_shriek` (Angry cat shriek / strike slap sound) | 4-frame claw cutscene plays for roughly 560ms with fade in/out and no red flash or screen shake. | "กดตอนแมวจ้อง! เสีย ♥ 1 ดวง" |
...
Cat Mood uses a generated floating bubble cue: sleepy uses `Z`, curious uses `?`, annoyed uses twitch marks, and angry uses a red-orange comic anger symbol. The cue is a UI layer that follows the head anchor without inheriting Cat Rig rotation or scale. Sleepy is shown only while HIDDEN; other levels are shown only during PEEK/WATCH. The cue is hidden during WARNING, ATTACK, SABOTAGE, and HIDE so Warning and the claw cutscene stay legible. The HUD keeps a compact four-segment Mood pill as fallback.
