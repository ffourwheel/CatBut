...
CatKub’s core loop relies on **high-stakes risk vs. reward timing** wrapped in a comforting **Cozy Cat Café** aesthetic.  
...
1. **Visual Cues**: Clear contrast shifts, radial progress fills, comic warning bubbles, screen shakes, and celebratory particle bursts.
...
| :--- | :--- | :--- | :--- | :--- |
| **OFF**<br>`(Idle / Ready)` | `btn_off` texture. Warm caramel wood base with soft ambient glow. Radial progress ring at 0%. | `sfx_button_tap_soft` (Cozy wooden click on first touch) | Idle subtle breathing scale `1.0 ↔ 1.02` (loop 1600ms, Sine.easeInOut). | "กดค้างไว้!" |
| **HOLDING**<br>`(Charging)` | `btn_holding` texture. Glowing matcha green radial ring filling 0% → 100%. Button visibly depresses. Micro ripple particles. | `sfx_charge_loop` (Ascending pitch café bubble boil / purr pitch climbing from 220Hz to 880Hz) | Scale down to `0.94` with spring ease (`Back.easeOut`, 120ms). Continuous micro-haptic vibration (15ms every 100ms). | "กำลังชาร์จ..." |
| **DECAY**<br>`(Early Release)` | Radial ring flashes amber/orange and rapidly unwinds backwards to 0%. Button pops back out. | `sfx_decay_whoosh` (Descending air whistle / soft deflation) | Scale springs back to `1.0` with overshot bounce (`Elastic.easeOut`, 250ms). Slight horizontal shake (±2px, 150ms). | "ปล่อยแล้ว! พลังลด!" |
| **ON**<br>`(Completed)` | `btn_on` texture. Golden paw print center emblem. Solid gold ring outline. Emits 8–12 golden star particles outwards. | `sfx_button_complete` (Crisp golden café order bell ding / chime) | Punch scale punch to `1.15` then settles to `1.0` (`Back.easeOut`, 200ms). Golden ring pulse. | "สำเร็จ! 1/4" (นับตามปุ่ม) |
| **SABOTAGED**<br>`(Cat Swipe)` | Button texture dims, red claw scratch mark flashes across face. Progress resets to 0%. Button temporarily disabled (300ms). | `sfx_cat_sabotage_swipe` (Mischievous cat paw swipe + soft wood rattle) | Rapid rotational wobble: `[-8°, +8°, -4°, 0°]` over 240ms (`Sine.easeInOut`). Red flash tint `#F44336` for 180ms. | "โดนแมวกวนแล้ว!" |
...
| **WATCH**<br>`(DANGER)` | `cat_hole_watch` texture. Cat fully raised, piercing direct glare. Golden eye glint FX. **High Danger: Holding buttons triggers instant strike!** | `sfx_cat_watch_stinger` (Tense violin / pizzicato sharp stinger + tense cat growl) | Instant lock freeze. Cat scale scales up slightly to `1.025` for intimidation (`Power2.easeOut`, 80ms). Pulsing red vignette around screen edge. | "แมวจ้องอยู่! ปล่อยมือเดี๋ยวนี้!" |
| **ATTACK** | `cat_hole_attack` texture. Fast lunge forward with claws extended. Screen flashes red. Heart lost in HUD. | `sfx_cat_attack_shriek` (Angry cat shriek / strike slap sound) | Full camera shake: `intensity = 0.025, duration = 300ms`. Red screen flash (`#F44336`, alpha 0.45 fade out over 250ms). | "โดนแมวจับได้แล้ว!" |
...
| **TUTORIAL** | Micro-Tutorial Modal | 3 swipeable/clickable cards:<br>1. **กดค้างชาร์จปุ่ม** (Hold to charge)<br>2. **แมวมองให้ปล่อยมือ** (Release when watched)<br>3. **ครบ 4 ปุ่มชนะ!** (Complete all 4 to win) | `sfx_tutorial_page_flip` (Soft paper rustle / card flip) | **เข้าใจแล้ว! ลุยเลย** (`btn_tutorial_ready`) |
| **GAMEPLAY** | In-Game HUD | Top Bar: Hearts (`3/3`), Score display, Combo badge (`×1`..`×5`), Progress indicator (`0/4`..`4/4`), Pause button, Sound toggle. | In-game dynamic audio mix | Interactive game loop |
...
1. **Cat Warning to Watch Grace Period**:
   ...
   - If player releases button within this window, no strike occurs.
2. **Watch Reaction Forgiveness**:
   - When `WATCH` activates, provide a **150ms human reaction forgiveness window** before triggering `ATTACK` if the player is still holding a button.
3. **Audio-Visual Haptic Parity**:
   - On mobile devices supporting the Web Vibration API (`navigator.vibrate`), fire a `25ms` light vibration on button press, `10ms` continuous pulse while holding, and a heavy `100ms` pulse on `ATTACK`.
