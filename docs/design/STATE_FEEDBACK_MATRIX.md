...
| **ATTACK** | `cat_hole_attack` texture. Fast lunge forward with claws extended. A cute generated paw/claw cutscene plays over a soft vignette, then the damaged heart reacts in the HUD. | `sfx_cat_attack_shriek` (Angry cat shriek / strike slap sound) | Camera shake `intensity = 0.014, duration = 250ms`; coral flash fades over 180ms; 4-frame claw cutscene plays for roughly 560ms. | "กดตอนแมวจ้อง! เสีย ♥ 1 ดวง" |
